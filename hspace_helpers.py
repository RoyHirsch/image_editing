        ...
        **kwargs,
        ...
        with self.pipe.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                if self.pipe.do_classifier_free_guidance and self.guidance_rescale > 0.0:
...
class SD3PipelinewWithHspace():
    ...
    def __call__(
        ...
        if 'prompt_embeds' in self.h_space_layer_names:
            latents_mem['prompt_embeds'] = prompt_embeds.clone().detach().cpu()

        if 'pooled_prompt_embeds' in self.h_space_layer_names:
            latents_mem['pooled_prompt_embeds'] = pooled_prompt_embeds.clone().detach().cpu()
        ...
        if 'noise' in self.h_space_layer_names:
            latents_mem['noise'] = latents.clone().detach().cpu()

        # 5. Prepare timesteps
        scheduler_kwargs = {}
        if self.pipe.scheduler.config.get("use_dynamic_shifting", None) and mu is None:
            _, _, height, width = latents.shape
            image_seq_len = (height // self.pipe.transformer.config.patch_size) * (
                width // self.pipe.transformer.config.patch_size
            )
            mu = calculate_shift(
                image_seq_len,
        ...
        if (ip_adapter_image is not None and self.pipe.is_ip_adapter_active) or ip_adapter_image_embeds is not None:
        ...
        ############ Modifications ############
        def get_latent_from_block(module, input, output, name):
            # context is the text + time embedding
            latents_mem[name + '_context'].append(output[0][1].clone().detach().cpu())
            # latents is the latent tokens embedding (64x64)
            latents_mem[name + '_latents'].append(output[1][1].clone().detach().cpu())

        # 7. Denoising loop
        hooks = {}
        for name in self.h_space_layer_names:
            if 'block' in name:
                _, num = name.split('_')
                num = int(num)
                latents_mem[name + '_context'] = []
                latents_mem[name + '_latents'] = []
                hooks[name] = self.pipe.transformer.transformer_blocks[num].register_forward_hook(
                    partial(get_latent_from_block, name=name))
            ...
            for i, t in enumerate(timesteps):
                ...
                if self.do_classifier_free_guidance:
                    ...
                    if skip_guidance_layers is not None and should_skip_layers:
                        ...
                        noise_pred_skip_layers = self.pipe.transformer(
                            hidden_states=latent_model_input,
                            timestep=timestep,
...
def get_pipeline_with_h_space(model_name, pipe, h_space_layer_names=['down_zero', 'down_one', 'down_two', 'down_three', 'mid']):
    ...
    if model_name == 'sd3':
        return SD3PipelinewWithHspace(pipe, h_space_layer_names)
    else:
